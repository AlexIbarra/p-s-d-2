...
int main(int argc, char** argv)
{
  ...
  for(i = 0; i < p; i++) {
    for(j = 0; j < q; j++) { 
      if(j%size != 0) { // Para evitar que el P0 se lo envie a si mismo
        ...
        if(rank == 0) {
          MPI_Isend(&second[i*q+j], 1, MPI_INT, j%size, 1, MPI_COMM_WORLD), &request[cont]);
          cont++;
        } 
        else if(rank == j%size) {
  ...
  if(rank == 0)
    MPI_Waitall(6, request_list, status_list);
    ...
    for(j = 0; j < q; j++){
      if(j%size != 0) {
        if (rank == j%size){
          ...
          MPI_Irecv(&multiply[i*n+j], 1, MPI_INT, j%size, 1, MPI_COMM_WORLD, &request[cont]);
          cont++;
        }
      }
    }
